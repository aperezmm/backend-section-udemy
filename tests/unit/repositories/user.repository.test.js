...
describe("User Repositories Tests", () => {
    ...
    it("Should return a user by id", async() =>{
        const _user = {...user}; //Copiamos el objeto nuevo pero sin referencia
        delete _user.password;
        mockingoose(User).toReturn(user, "findOne");

        const _userRepository = new UserRepository({ User }); //Preparamos nuestro repositorio para trabajar
        const expected = await _userRepository.get(_user._id);

        //debemos parsear el expected por el formato que responde mongoose

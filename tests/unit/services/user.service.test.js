...
describe("User service Tests", () => {
    ...
    it("Should find a user by id", async () => {
        ...
        UserRepository.get.mockReturnValue(user); //El valor que va a retornar dicha funcion

        const _userService = new UserService({ UserRepository });
        const expected = await _userService.get(user._id);
        expect(expected).toMatchObject(user); //Se espera que el resultado del expected sea igual al user que le estamos pasando en linea 14
    });
